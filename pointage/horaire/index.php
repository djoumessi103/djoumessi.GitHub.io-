...
try {
    ...
    // Initial query to fetch all schedules with employee names
    $stmt = $pdo->prepare("
        SELECT
            h.*,
            e.nom AS employe_nom
        FROM horaire h
        JOIN employe e ON h.id_employe = e.id_employe
    ");
    $stmt->execute();
    $horaires = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    <div class="employee-list-container">
        <div class="employee-list-wrapper">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <a href="horaire.php" class="btn btn-success">
                ...
                <div class="input-group search-input-group">
                    <input type="text" class="form-control" placeholder="Rechercher un horaire..." aria-label="Rechercher un horaire" aria-describedby="button-addon2">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php
                    if ($horaires != null && sizeof($horaires) > 0):
                        $i = 0;
                        foreach ($horaires as $horaire):
                            ...
                            <tr>
                                <td><?= $i ?></td>
                                <td><?= $horaire->employe_nom ?></td>
                                <td><?= $horaire->date ?></td>
                                <td><?= $horaire->heure_arrivee ?></td>
                                <td><?= $horaire->heure_depart ?></td>
                                <td class="options">
                                    <a href="modifier.php?id=<?= $horaire->id_horaire ?>" class="btn btn-primary btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir modifier cette horaire?')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
                                            <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325"/>
                                        </svg>
                                    </a>
                                    <a href="delete.php?id=<?= $horaire->id_horaire ?>" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette horaire ?')">

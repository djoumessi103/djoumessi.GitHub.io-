...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        ORDER BY p.date DESC, p.heure_pointage DESC
    "); // Added ORDER BY for better display
    $stmt->execute();
    $pointages = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    <style>
        ...
        * {
        ...
        .employee-list-wrapper {
            width: 100%;
            max-width: 1200px;
            background-color: #f8f9fa; /* Added background to wrapper */
            padding: 20px;
            border-radius: 8px;
        ...
        .employee-list-wrapper .table th, .employee-list-wrapper .table td {
            ...
            padding: 0.8rem; /* Increased padding for readability */
            text-align: left;
                ...
                <ul>
                    ...
                    <li>
                       <a href="index.php" class="active">
                            <span class=""></span> <small>Liste des pointages</small>
                        </a>
                    </li>
                </ul>
            </div>
    ...
    <div class="employee-list-container">
        <div class="employee-list-wrapper">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <a href="pointage.php" class="btn btn-success">
                ...
                <div class="input-group search-input-group">
                    <input type="text" class="form-control" placeholder="Rechercher un pointage..." aria-label="Rechercher un pointage" aria-describedby="button-addon2">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                        if ($pointages != null && sizeof($pointages) > 0):
                            $i = 0;
                            foreach ($pointages as $pointage):
                                ...
                                <tr>
                                    <td><?= $i ?></td>
                                    <td><?= htmlspecialchars($pointage->employe_nom) ?></td>
                                    <td><?= htmlspecialchars($pointage->date) ?></td>
                                    <td><?= htmlspecialchars($pointage->heure_pointage) ?></td>
                                    <td><?= htmlspecialchars($pointage->type_pointage) ?></td>
                                    <td class="options">
                                        <a href="modifier.php?id=<?= htmlspecialchars($pointage->id_pointage) ?>" class="btn btn-primary btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir modifier ce pointage ?')">
                                        ...
                                        <a href="delete.php?id=<?= htmlspecialchars($pointage->id_pointage) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce pointage ?')">

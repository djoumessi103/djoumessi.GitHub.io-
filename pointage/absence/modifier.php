...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_GET['id'])) { // Check if we are modifying
        $id = $_GET['id'];
        $stmt = $pdo->prepare("UPDATE absence SET id_employe = ?, date_debut = ?, date_fin = ?, type_absence = ? WHERE id_absence = ?");
        $stmt->execute([$id_employe, $date, $date_fin, $type_absence, $id]);
    } else { // If not modifying, it's an insert
        $stmt = $pdo->prepare("INSERT INTO absence (id_employe, date_debut, date_fin, type_absence) VALUES (?, ?, ?, ?)");
        $stmt->execute([$id_employe, $date, $date_fin, $type_absence]);
    }
...
<head>
    ...
    <title><?php echo isset($_GET['id']) ? 'Modifier absence' : 'Ajouter absence'; ?></title>
...
<input type="checkbox" id="menu-toggle">
            ...
            <div class="side-menu">
                <ul>
                    ...
                    <li>
                        <a href="index.php">
                            <span class=""></span>
                            <small>Liste abences</small>
                        </a>
                    </li>
                    <li>
                        <a href="absence.php">
                            <span class=""></span>
                            <small>Ajouter absences</small>
                        </a>
                    </li>
                    <li>
                        <a href="">
    ...
    <div class="content-wrapper">
        <div class="container mt-5 form-container">
            <center><h1><?php echo isset($_GET['id']) ? 'Modifier l\'absence' : 'Ajouter une absence'; ?></h1></center>
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_employe" class="form-control">
                            <?php foreach ($employes as $employe): ?>
                                <option value="<?php echo $employe['id_employe']; ?>" <?php echo (isset($absence['id_employe']) && $absence['id_employe'] == $employe['id_employe']) ? 'selected' : ''; ?>><?php echo $employe['nom']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="date_debut">Date debut :</label>
                        <input type="date" name="date_debut" class="form-control" value="<?php echo isset($absence['date_debut']) ? $absence['date_debut'] : ''; ?>">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="date_fin">Date fin :</label>
                        <input type="date" name="date_fin" class="form-control" step="0.01" value="<?php echo isset($absence['date_fin']) ? $absence['date_fin'] : ''; ?>">
                    </div>
                    <div class="form-group">
                        <label for="type_absence">Type absence :</label>
                        <input type="text" name="type_absence" class="form-control" step="0.01" value="<?php echo isset($absence['type_absence']) ? $absence['type_absence'] : ''; ?>">
                    </div>
                </div>
